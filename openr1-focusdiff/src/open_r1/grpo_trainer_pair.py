...
from models.janus.models import VLChatProcessor
...
def get_prob(step, s1=50, s2=450, s3=1200, prob=0.25):
    if step<=s1:
        return 1.0
    elif step<=s2:
        return prob + (1.0-prob)*(s2-step)/(s2-s1)
    elif step<=s3:
        return prob - prob*(step-s2)/(s3-s2)
    else:
        return 0.0
...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if processing_class is None:
            ...
            processor_path = 'path to deepseek-ai/Janus-Pro-7B'
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        if 'imag' in inputs[0].keys() and inputs[0]['imag'] is not None:
            if random.random() > get_prob(self.state.global_step):
                ...
                bad_img = []
                for inp in inputs:
                    good_img.append(Image.open(inp['imag'][0]).convert('RGB'))
                    good_img.append(Image.open(inp['imag'][1]).convert('RGB'))
                    bad_img.append(Image.open(inp['imag'][1]).convert('RGB'))
                    bad_img.append(Image.open(inp['imag'][0]).convert('RGB'))
        ...
        with torch.no_grad():
            if good_img is not None:
                ...
                pixel_values = torch.stack(good_img_transform, dim=0).to(torch.bfloat16).cuda()
                ...
                pixel_values = torch.stack(bad_img_transform, dim=0).to(torch.bfloat16).cuda()
                _, _, all_image_ids = self.model.gen_vision_model.encode(pixel_values)
        ...
        for prompt in prompts:
            if 'pro' not in self.model_name.lower():
                conversation = [
                    {
                        "role": "User",
                        "content": prompt,
        ...
        if self.args.use_vllm:
            ...
            with unwrap_model_for_generation(self.model, self.accelerator) as unwrapped_model:
                ...
                img_ids, imgs, (text_ids, all_attention_mask) = unwrapped_model.generate_image(
        ...
        if good_img_id is not None:
            ...
            for k in range(0,len(imgs),self.num_generations):
                all_imgs += imgs[k:k+self.num_generations]
        ...
        if good_img_id is not None:
            ...
            img_ids_list.append(good_img_id.unsqueeze(1))
            txt_ids_list.append(new_text_ids)
            attn_mask_list.append(new_atten_mask)

            img_ids_list.append(bad_img_id.unsqueeze(1))
            txt_ids_list.append(new_text_ids)
            attn_mask_list.append(new_atten_mask)
            ...
            text_ids = torch.cat(txt_ids_list, dim=1)
        ...
        with torch.no_grad():
            ...
            if not self.apply_api:
                output_reward_func = self.reward_funcs[0](self.reward_model, images=imgs, prompts=prompts, apply_api=self.apply_api)
                rewards_per_func[:, 0] = torch.tensor(output_reward_func, dtype=torch.float32, device=device)

            else:
                all_imgs = gather_object(imgs)
                allprompts = gather_object(prompts)
                if self.accelerator.is_main_process:
                    score1 = self.reward_funcs[0](None, all_imgs, allprompts, self.apply_api)
                else:
        ...
        # Log the metrics
        # 分布式训练，将所有设备的reward_per_func进行整合，最后可获得num_gpus * batchsize 个reward
